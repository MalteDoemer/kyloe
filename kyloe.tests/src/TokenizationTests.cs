...
namespace Kyloe.Tests.Tokenization
{
    public class TokenizationTests
    {
        ...
        public void Test_Tokenization_With_Errors(string text, DiagnosticKind[] kinds)
        {
            var (_, diagnostics) = SyntaxTree.Tokenize(text);

            DiagnosticAssert.HasAll(diagnostics, kinds);
        }
        ...
        public static IEnumerable<object[]> GetSimpleTokenData()
        {
            ...
            {
                var text = SyntaxInfo.GetSimpleTokenString(kind);
        ...
        public static IEnumerable<object[]> GetCombinedTokenData()
        {
            ...
            foreach (var kind1 in kindArray)
            {
                foreach (var kind2 in kindArray)
                {
                    ...
                    var text1 = SyntaxInfo.GetSimpleTokenString(kind1);
                    var text2 = SyntaxInfo.GetSimpleTokenString(kind2);
